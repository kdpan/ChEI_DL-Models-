...
def train(model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        _,loss = binary_cross_entropy(output, graph.y.float().to(device))
        loss.backward()
        optimizer.step()
...
def train_init(dataset='che',pretrain=None):
    ...
    valid_data = LMsGraphDataset('data/{}_test.csv'.format(dataset),smile2graph,seq2path)
    ...
    return model, device, train_loader, test_loader, valid_loader, optimizer, graphNet, dataset
...
pretrain = False
model, device, train_loader, test_loader, valid_laoder, optimizer, graphNet, dataset = train_init(pretrain=pretrain)
...
for epoch in range(NUM_EPOCHS):
    ...
    print('epoch:{}\tauroc:{:.4f}\tauprc:{:.4f}'.format(epoch,auroc,auprc))
